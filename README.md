...
The Community section is integrated into the main site's navigation, header, footer, and CSS, using the same restrained landmark structure as the rest of the site (Banner, Primary navigation, Main, Footer only — no region per heading). Registration is **live through OpenDoorDesign.org**. Railway hosts the production Community backend, registrations are stored in SQLite, and provider-backed confirmation email delivery is active through Resend. Successful registrants are directed into the Community Dashboard while real sign-in, connected profiles, discussions, and other member features remain future work. It includes:
...
- `community/welcome.html` — registration confirmation. Confirms receipt, tells the registrant that a confirmation email has been sent, identifies Collaborate@community.opendoordesign.org as the Community sender, and links to the Community Dashboard.
