...
function buildApp(config, store = getRegistrationStore(config)) {
  ...
  // required because GitHub Pages continues to serve the HTML directly.
  // Leaving it enabled keeps local development simple.
  ...
  // all unless ADMIN_ENABLED=true. createAdminRouter also requires valid
  // admin credentials before it can be created.
  if (config.admin && config.admin.enabled) {
    app.use("/community/admin", createAdminRouter(config, store));
  }
    ...
    async (request, response) => {
      ...
      if (!valid) {
        ...
        response
          .status(400)
          .type("html")
          .send(renderRegisterPageWithErrors(errors, value));
      ...
      if (
        ...
        // submissions return the same neutral welcome experience without
        // creating another row or sending another confirmation email.
        store.recordEvent({
        ...
        response.redirect(
          303,
          "https://opendoordesign.org/community/welcome.html"
        );
      ...
      try {
        ...
        // eslint-disable-next-line no-console
        console.error(
          `[community-registration] storage failure (${correlationId}):`,
          error
        );
        ...
        response.status(500).type("html").send(
          renderRegisterPageWithErrors(
            [{
              field: "first-name",
              message:
                "Registration was not completed because of a technical problem. Please try again, or contact Info@OpenDoorDesign.org."
            }],
            value
          )
        );
      ...
      // only logged and recorded, per Feature 003, Phase 4.
      try {
        ...
        store.recordEvent({
          ...
          result: emailResult.sent
            ? "sent"
            : `not_sent_${emailResult.provider}`,
          correlationId
        });
      } catch (emailError) {
        store.recordEvent({
        ...
        // eslint-disable-next-line no-console
        console.error(
          `[community-registration] confirmation email failed (${correlationId}):`,
          emailError
        );
      }
